...
/*
    einstell read/write schnittstelle input.csv [output.csv]
...
void handle_args(int argc, const char* argv[]) {
    ...
    if( (argc == 2) && !(string(argv[1]).compare("--help")) ) {
        ...
        cout << "einstell write/read  schnittstelle input.csv [output.csv]" << endl;
...
void Einstelltabelle::read_csv() {
    ...
    while(true) {
        buffer = get(csv_in);
        if( (buffer.compare(0, 7, "Regler,") == 0) ) {
            break;
        } else {
            comment += buffer + "\n";
        }
    ...
    //Hole Regleradresse aus dem Stream
    sscanf(buffer.substr(7).c_str(), "%d", &g_regleradresse);
...
void Einstelltabelle::write_csv() {
    (*csv_out) << comment;
    (*csv_out) << "Regler," << g_regleradresse << endl;
